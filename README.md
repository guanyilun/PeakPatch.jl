...
Both runs use Websky cosmology (Planck 2018), `use_lcg=true`,
`fortran_compat=true`, seed=13579, top-hat filters, 2LPT, `rmax2rs=0.0`.

**256³** (boxsize=618 Mpc/h, 16 filters):

| Metric | Fortran | Julia | Diff |
|--------|---------|-------|------|
| Density σ | 2.728134 | 2.728007 | 0.005% |
| Total peaks | 78,734 | 80,243 | +1.9% |
| Total halos | 67,771 | 68,041 | **+0.4%** |

**1024³** (boxsize=535 Mpc/h, same volume at 4.6× finer resolution, 16 filters):

| Metric | Fortran | Julia | Diff |
|--------|---------|-------|------|
| Density σ | 4.537039 | 4.536786 | 0.006% |
...
The smoothed fields agree to 0.005% in aggregate (σ), but individual cells
differ by ε ≈ 0.01 in field units (~0.6% of σ) due to different computation
paths: Julia uses Float64 log-log P(k) interpolation with a single IRFFT,
while Fortran uses Float32 linear interpolation with an MPI
IFFT→SlabToCube→serial FFT→smooth→serial IFFT round-trip. Cells near the
collapse threshold (δ ≈ 1.686) can land on different sides in each code.

The expected number of "flipped" peaks per filter is:

    N_flip ≈ N_cells × p(fcrit) × ε × f_peak

where p(fcrit) is the Gaussian PDF at threshold and f_peak is the fraction
of above-threshold cells that are local maxima. For R=3.985 at 1024³
(σ=1.56): N_flip ≈ 941M × 0.143 × 0.01 × 8.7×10⁻⁴ ≈ 1,170, vs the
observed difference of 1,816 — correct order of magnitude. Summed across
all filters, this gives the observed ~2% total peak difference.

After shell analysis, most flipped peaks are rejected: a peak barely above
threshold rarely passes the radial collapse criterion (which integrates over
thousands of cells), reducing the difference to **0.4–1.4% in total halos**.

| Source | Impact | Notes |
|---|---|---|
| P(k) interpolation | 0.005% in σ | Julia: Float64 log-log; Fortran: Float32 linear |
| FFT path difference | ~0.6% of σ per cell | Different FFTW plans and round-trip count |
| Per-filter peaks | 1–33% | Threshold-crossing jitter, worst at high ν (few peaks) |
| Total halos | 0.4–1.4% | Shell analysis is robust to per-cell noise |

### Performance

Julia was initially much slower than Fortran at 1024³ because FFTW defaulted
to 1 thread and all k-space loops were serial. After adding
`FFTW.set_num_threads()` and `Threads.@threads` on the outer z-loop of the
k-space kernels (matching Fortran's `!$OMP PARALLEL DO`), Julia 1024³ dropped
from >2.5 hours to 18 minutes — 3× faster than Fortran because Julia applies
the smoothing window directly in k-space with a single IRFFT, while Fortran
does an extra FFT round-trip per field for MPI data redistribution (even at
ntile=1).
