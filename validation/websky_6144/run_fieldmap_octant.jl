...
#
# Usage: julia --project=. -t 32 run_fieldmap_octant.jl <config.toml> [nside] [outdir]
...
function main()
    ...
    @info "fieldmap octant" tag nside npix devices seed ntile coarse_factor chi_star
    t0 = time()
    maps = run_multitile_fieldmap(cfg; ntile=ntile, seed=seed, coarse_factor=coarse_factor,
                                  ...
                                  chi_star=chi_star, subdiv_max=3,
                                  use_gpu=true, devices=devices, verbose=true)
    elapsed = time() - t0
